        ...
        if Ty.compare tx' ty' = 0 then begin
          ...
          match tx', ty' with
            ...
            | Ty.Tuple (_, tys1), Ty.Tuple (_, tys2) ->
                ...
                with Invalid_argument str as e ->
                  match str with
                  | "List.fold_left2" -> Result.Error (EzyErrors.ArityClash (tx', ty', List.length tys1, List.length tys2), locs, l)
                  | _ -> raise e
                end
...
let solve env max_time cs pp _ast: (TyVarSubst.t * EzyErrors.ErrorSet.t, EzyErrors.ErrorSet.t) Result.t =
...
let type_expression env _program (expr: EzyAst.imported_expression) ast =
...
(* abeliangrape: silence unused warning *)
let () = (fun _ -> ()) type_expression
            
let rec print_strs_info s env ppf = function
  | [] -> ()
  | { EzyAst.pstr_desc = EzyAst.Pstr_eval expr ; _ } :: rem ->
  ...
  | { EzyAst.pstr_desc = EzyAst.Pstr_rec_value bindings ; _ } :: rem ->
      let pp (var, _) =
      ...
      print_strs_info s env ppf rem
  | { EzyAst.pstr_desc = EzyAst.Pstr_value bindings ; _ } :: rem ->
      let pp = function
        | { EzyAst.ppat_desc = EzyAst.Ppat_var var ; _ }, _ ->
            let vd = EzyEnv.find_value (Path.Pident var.EzyAst.nm_data) env in
            let ty' = TyVarSubst.apply_to_ty s vd.EzyEnv.val_ty in
      ...
      List.iter pp bindings ;
      print_strs_info s env ppf rem
  | { EzyAst.pstr_desc = EzyAst.Pstr_type tds ; _ } :: rem ->
      Format.fprintf ppf "@[type %a@]@ " (List.print (EzyAst.print_name ()) " and ") (List.map fst tds) ;
      print_strs_info s env ppf rem
  | { EzyAst.pstr_desc = EzyAst.Pstr_open nm ; _ } :: rem ->
...
let type_structure ?program (oenv: Env.t) (sstr: EzyAst.imported_structure) =
  let () = (fun _ -> ()) program in (* quiet unused variable warning. ~abeliangrape *)
...
let type_and_compare_implementation sourcefile outputprefix modulename initial_env parse_tree fs =
  (* Not sure where this has gone, but Config.load_path no longer exists. ~abeliangrape
  ...
  *)
  ...
  begin try
    let tt = Typemod.type_implementation sourcefile outputprefix modulename initial_env parse_tree in
    begin match EzyEnrichedAst.eq_structure s enr_str tt.structure.str_items with
      | Some msg -> alpha_error msg
      | None -> ()
    end ;
    ted_str, (tt.structure, tt.coercion)
  with
    | Typemod.Error (loc, env, err) ->
        beta_error loc Location.print_report (Typemod.report_error ~loc env err)
    | Typecore.Error (loc, env, err) ->
        beta_error loc Location.print_report (Typecore.report_error ~loc env err)
  end


let type_and_compare_top_phrase fs oldenv str =
  (* Same as above. ~abeliangrape
  logger#debug "Conf.load_path: [%a]" (EzyUtils.List.print Format.pp_print_string ", ") (!Config.load_path);
  *)
  let enr_str, s, _env =
  ...
  begin try
    ...
    let (str, sg, _names, newenv) = Typemod.type_structure oldenv str in
    Typecore.force_delayed_checks ();
    begin match EzyEnrichedAst.eq_structure s enr_str str.str_items with
      | Some msg -> alpha_error msg
      | None -> ()
    end ;

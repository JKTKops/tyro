...
let enrich_name ?ident nm = 
  let data =
    match ident with
      | None -> Ident.create_local nm.nm_name
      | Some id -> id in
  {
    nm_name = nm.nm_name ;
    nm_loc = nm.nm_loc ;
...
and core_type_desc = Parsetree.core_type_desc =
  ...
  | Ptyp_object of Parsetree.object_field list * EzyAsttypes.closed_flag
  | Ptyp_class of Longident.t loc * core_type list
  | Ptyp_alias of core_type * string
  | Ptyp_variant of Parsetree.row_field list * closed_flag * label list option
...
and ('expr_data, 'id_data, 'nm_data, 'pat_data) pattern_desc =
  ...
  | Ppat_tuple of ('expr_data, 'id_data, 'nm_data, 'pat_data) pattern list
    (* we can probably get rid of the bool here entirely to match compilerlibs *)
...
and ('expr_data, 'id_data, 'nm_data, 'pat_data) rule =
  { rule_pat: ('expr_data, 'id_data, 'nm_data, 'pat_data) pattern;
    rule_guard: ('expr_data, 'id_data, 'nm_data, 'pat_data) expression option;
    rule_exp: ('expr_data, 'id_data, 'nm_data, 'pat_data) expression;
  }
...
let rec print_core_type ppf ct =
  match ct.ptyp_desc with
    ...
    | Ptyp_constr (k, cts) ->
        ...
        fprintf ppf "%t%a" aux Longident.print k.txt
    | Ptyp_poly ([], ct) ->
...
let print_name ?nap () ppf name =
  (fun _ -> ()) nap; (* can't change name to _nap since it's in the interface ~abeliangrape*)
...
let print_pat, print_expr, print_structure_item = 
  let aux ?eap ?iap ?nap ?pap () =
    ...
    (* can't change names to _XXX because they are in the interface ~abeliangrape *)
    (fun _ -> ()) (iap, nap, pap);
    ...
    and print_expr ppf expr =
      let print_expr_desc ppf = function
        | Pexp_ident { lid_name = v ; _ } ->
            Longident.print ppf v
        | Pexp_constant c ->
            print_constant ppf c
        | Pexp_let (bindings, body) ->
            let rec collect_funs sofar = function
              | { pexp_desc = Pexp_function ([rule]) ; _ } ->
                  collect_funs (rule.rule_pat :: sofar) rule.rule_exp
                (* Trying to understand this:
                   if we have a function with multiple rules, we're not interested,
                   I think, since we'll have to print it with `function' anyway.
                   ~abeliangrape *)
        ...
        | Pexp_apply ({ pexp_desc = Pexp_apply ({ pexp_desc = Pexp_ident {lid_name = Longident.Lident nm;_} ; _ }, exp1) ; _ }, exp2)
            when is_infix nm ->
        ...
        | Pexp_construct ({ lid_name = lid ; _ }, opt_exp, _) ->
              ...
              | Longident.Lident "::" ->
                  ...
                  let rec print_list ppf sofar expr =
                    match expr.pexp_desc with
                      | Pexp_construct ({lid_name = Longident.Lident "[]";_}, None, _) ->
                          print_proper_list ppf (List.rev sofar)
                      | Pexp_dots _ ->
                          print_proper_list ppf (List.rev (expr :: sofar))
                      | Pexp_construct ({lid_name = Longident.Lident "::";_},
                                        Some ({pexp_desc = Pexp_tuple [head; tail];_}), _) ->
    ...
    and print_value_binding ppf (pat, exp) =
        fprintf ppf "@[%a ->@ %a@]" print_pat pat print_expr exp

    and print_rule ppf rule =
      match rule.rule_guard with
      | None -> print_value_binding ppf (rule.rule_pat, rule.rule_exp)
      | Some g -> fprintf ppf "@[%a when@ %a ->@ %a@]"
          print_pat  rule.rule_pat
          print_expr g
          print_expr rule.rule_exp
...
module CollectLocs = struct
  ...
  let rec core_type ct =
    add ct.ptyp_loc begin
      match ct.ptyp_desc with
        ...
        | Ptyp_variant _ | Ptyp_package _ | Ptyp_extension _ -> empty
  ...
  let rec expr e =
    add e.pexp_loc begin
      match e.pexp_desc with
        ...
        | Pexp_dots _ -> empty
        | Pexp_let (vbs, body) ->
            union (value_bindings vbs) (expr body)
  ...
  and value_bindings vbs = big_union (List.map (on_pair pattern expr) vbs)
  and rules rs =
    let on_rule rule =
      [pattern rule.rule_pat; on_opt expr rule.rule_guard; expr rule.rule_exp]
    in big_union (List.concat_map on_rule rs)
  ...
  let strit s = 
    add s.pstr_loc begin
      match s.pstr_desc with
        | Pstr_eval e -> expr e
        | Pstr_value vbs -> value_bindings vbs
...
module MapAstData = struct
  ...
  and expression map_expr_data map_pattern_data expr =
    ...
    let pat_aux = pattern map_expr_data map_pattern_data in
    let vbs_aux vbs = List.map (fun (p, e) -> pat_aux p, exp_aux e) vbs in
    let rule_aux r = {
      rule_pat = pat_aux r.rule_pat;
      rule_guard = begin match r.rule_guard with | None -> None | Some e -> Some (exp_aux e) end;
      rule_exp = exp_aux r.rule_exp;
    } in
    let rls_aux rls = List.map rule_aux rls in
    let desc' = match expr.pexp_desc with
      | Pexp_ident _ 
      | Pexp_assertfalse 
      | Pexp_constant _ as desc -> desc
      | Pexp_let (bindings, body) ->
          Pexp_let (vbs_aux bindings, exp_aux body)
      | Pexp_letrec (bindings, body) ->
      ...
      | Pexp_setfield (e1, lid, e2) ->
          Pexp_setfield (exp_aux e1, lid, exp_aux e2)

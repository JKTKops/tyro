...
let rec mask_item = function
  | Dot_str {pstr_desc = Pstr_dots il ; _}
  | Dot_exp {pexp_desc = Pexp_dots il ; _} 
  | Dot_pat {ppat_desc = Ppat_dots il ; _}  -> mask il
  | s -> [s]
...
let get_location_list_of_pattern_tuple pos pel =
  List.fold_left
    (fun ls { ppat_loc = pl ; _ } -> pl :: ls)
    []
    (List.map pos pel) 
    
let get_location_list_of_name_X_tuple nel = 
  List.fold_left 
    (fun ls { nm_loc = nl ; _ } -> nl :: ls) 
    [] 
    (List.map (fun (n,_) -> n) nel)  
    
let get_location_list_of_expression el =
  List.map (fun { pexp_loc = l ; _ } -> l) el

let get_location_list_of_pattern pl =
  List.map (fun { ppat_loc = l ; _ } -> l) pl
    
let get_location_list_of_x_expression_tuple el =
  List.fold_left
    (fun ls { pexp_loc = l ; _ } -> l :: ls)
    []
    (List.map snd el) 

let is_rule_dot r =
  match (r.rule_pat, r.rule_guard, r.rule_exp) with
  | ( {ppat_desc = Ppat_dots [] ; _}
    , (None | Some {pexp_desc = Pexp_dots [] ; _ })
    , {pexp_desc = Pexp_dots [] ; _}
    ) -> true
  | _ -> false

let is_pedot_tuple (p,e) =
  match (p,e) with
    | ({ppat_desc = Ppat_dots [] ; _},{pexp_desc = Pexp_dots [] ; _}) -> true
    | _ -> false

let is_nedot_tuple ({nm_loc = l;_},e) lset =
  match e with
    | {pexp_desc = Pexp_dots [];_} -> LocationSet.mem l lset
    | _ -> false
        ...
        | Pstr_type ntdl -> 
            let llist = get_location_list_of_name_X_tuple ntdl in               
              do_if_one_member llist 
                ...
                (il_to_str [])  
        | Pstr_exception ({nm_loc = nl;_},_ct) -> 
    ...
    lset -> 
      ...
      let ident () = e_exp in
        ...
        match edesc with
          ...
          | Pexp_function rl ->
              let llist = get_location_list_of_pattern_tuple (fun r -> r.rule_pat) rl  in

              let rl_exps r = match r.rule_guard with
                | None -> [r.rule_exp]
                | Some g -> [g; r.rule_exp] in
              let sliced_rl = create_sliced_rules rl lset in 
                do_if_one_member (l::llist) 
                  (edesc_to_exp (Pexp_function sliced_rl))
                  (do_mask (List.concat_map rl_exps sliced_rl))
          ...
          | Pexp_match (e,rl) ->
              ...
              let sliced_e = create_sliced_exp e lset in
                do_if_one_member (l::llist) 
                  ...
                  (do_mask (sliced_e :: (List.concat_map rl_exps sliced_rl)))
          ...
          | Pexp_construct (i,eopt,b) -> 
              do_if_one_member [l]
                (edesc_to_exp 
                   (Pexp_construct 
                      (i,Option.map eopt ~f:(fun e -> create_sliced_exp e lset),b)))
                (do_mask ((function 
                               None -> [] 
          ...
          | Pexp_record (iel,eopt) ->
              let sliced_el = create_sliced_iel iel lset in 
                do_if_one_member [l] 
                  (edesc_to_exp (Pexp_record (sliced_el,(Option.map eopt ~f:(fun e -> create_sliced_exp e lset)))))
                  (do_mask (((function 
                                  None -> [] 
                                | Some e -> [create_sliced_exp e lset]) 
              ...
              let sliced_e2 = create_sliced_exp e2 lset in
                do_if_one_member [l]
                  (edesc_to_exp (Pexp_ifthenelse (sliced_e1,sliced_e2,
                                                  (Option.map eopt3 ~f:(fun e -> create_sliced_exp e lset)))))
...
and create_sliced_rules rll lset =
    magic_filter
      ~f:(fun r -> { rule_pat = create_sliced_pat r.rule_pat lset;
                     rule_guard = begin match r.rule_guard with
                      | None -> None 
                      | Some g -> Some (create_sliced_exp g lset)
                      end;
                     rule_exp = create_sliced_exp r.rule_exp lset;
                   })
      ~p:(fun r1 r2 -> is_rule_dot r1 && is_rule_dot r2)
      rll
...
and create_sliced_ntdl ntdl lset =
  magic_filter 
    ~p:(fun ({nm_loc = l1;_},_) ({nm_loc = l2;_},_) -> 

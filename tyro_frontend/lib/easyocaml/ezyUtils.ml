...
(* With compiler-libs, we can probably use jane street's Core instead,
   eventually. ~abeliangrape *)
...
let input_all t = (* copied from core 0.5 *)
  ...
  let input t ~buf ~pos ~len = Stdlib.input t buf pos len in
  let rec loop () =
...
(*let _atime : string -> ('a -> 'b) -> 'a -> 'b =
  ...
  class logger layer = object (_self)

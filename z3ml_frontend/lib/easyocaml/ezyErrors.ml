...
type fatal = 
    (* replaced with Syntaxerr.error in place of EzyOcamlGrammar.ParseError.error.
        I've deleted the whole EzyOcamlGrammar module
        in favor of potentially eventually replacing it with a ppx rewriter *)
  | Parse_error of Syntaxerr.error
...
module Error = struct
  ...
  type printable = t
  let print _ppf _ = failwith "no printer for Error.t"
  let compare =
    lexical2 Stdlib.compare Stdlib.compare
end
...
module HeavyError = struct
  ...
  let print _ppf _ = failwith "no printer for Error.t"
  let compare x y =
    lexical2 Stdlib.compare Stdlib.compare x y
...
let rough_ty_print =
  match lang with
    | `En -> begin fun ppf -> function
        ...
        | Ty.Constr (_label, path, _args) ->
            fprintf ppf "%s" (Path.name path)
      end 
    | `Fr -> begin fun ppf -> function
        | Ty.Var _ as ty ->
            Ty.print ppf ty
        | Ty.Tuple (_label, _) ->
            fprintf ppf "multiplet"
        | Ty.Arrow (_label, _, _) ->
            fprintf ppf "fonction"
        | Ty.Constr (_label, path, _args) ->
            fprintf ppf "%s" (Path.name path)
      end
...
let print_type_error_desc ~w =
  ...
  let (cclash_str, aclash_str, circ_str) =
    match lang with
      | `En -> ( "Type constructor clash between %a and %a"
               , "Arity clash, %a vs. %a"
               , "Circular type, %a vs. %a"
               )
      | `Fr -> ( "Conflit entre les constructeurs de types %a et %a"
               , "Conflit entre les arités, %a vs. %a"
               , "Type circulaire, %a vs. %a"
               )
      | `De -> ( "Kollision zwischen Werten der Typen %a und %a"
               , "Ein %a anstatt eines %a"
               , "Zirkulaerer typ: %a vs. %a"
               )
    in
  let (cclash, aclash, circ) =
    (* Note: raises Scan_failure if we make a mistake above *)
    ( Scanf.format_from_string cclash_str "%a%a"
    , Scanf.format_from_string aclash_str "%a%a"
    , Scanf.format_from_string circ_str "%a%a"
    ) in
...
let print_error_desc =
  match lang with
    ...
    | `Fr -> begin fun ppf -> function
        | Type_error (_err, _locs) ->
            fprintf ppf "erreur de type"
        | Missing_fields (tyctor, lis) ->
            fprintf ppf "Les champs %a du type d'enregistrement %a ne sont pas définis"
              (List.print pp_print_string ", ") lis Longident.print tyctor
        | Unknown_field lid ->
            fprintf ppf "Il n'y a pas d'enregistrement avec le champ s'appele %a"
              Longident.print lid          
        | Unbound_variable lid ->
            fprintf ppf "La variable %a n'est pas liée"
              Longident.print lid
        | Mutable_content lid ->
            fprintf ppf "Le champ d'enregistrement %a n'est pas mutable"
              Longident.print lid
        | No_subtype (tx, ty) ->
            fprintf ppf "Cette expression de type %a ne peut pas être contrainte au type %a"
              Ty.print tx EzyAst.print_core_type ty
      end
...
let _print_specific_parse_error_desc =
    (* give a crappy message for now because we won't even invoke ezy if we can't parse the code. *)
  fun _ppf _err -> failwith "print_specific_parse_erro_desc: made it here?"  

    (* these use the old camlp4 stuff
  match lang with
...
let print_parse_error_desc =
  (* see above *)
  fun _ppf _err -> failwith "print_parse_err_desc: made it here?"
  (*
  match lang with
    | `En | `Fr -> begin fun ppf err -> 
...
let print_valid_ref: (program:(string lazy_t) -> (Ty.t, Path.t, Ident.t, Ty.t) EzyAst.structure -> Format.formatter -> unit) ref =
  ref (fun ~program _ _ -> (fun _ -> ()) program; ())
...
let register name ?print_valid print_errors print_heavies print_fatal =
  ...
  print_valid_ref := Option.value 
    ~default:(fun ~program _ _ -> (fun _ -> ()) program; ()) print_valid;
